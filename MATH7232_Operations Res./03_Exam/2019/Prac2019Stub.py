...
# Variables
X = [m.addVar(vtype=GRB.BINARY) for f in F]
Y = {(c,f):m.addVar(vtype=GRB.BINARY) for c in C for f in F}

# Objective
m.setObjective(quicksum(Build[f]*X[f] for f in F) +
               quicksum(Assign[c][f]*Y[c,f] for c in C for f in F),
               GRB.MINIMIZE)
...
for c in C:
    for f in F:
        m.addConstr(X[f]-Y[c,f]>=0)


for f in F:
    m.addConstr(quicksum(Y[c,f] for c in C) >= 6)
    
for c in C:
    m.addConstr(quicksum(Y[c,f] for f in F) >= 1)


m.optimize()
print(m.objVal)

print([X[f].x for f in F])

for c in C:
    print([Y[c,f].x for f in F])
