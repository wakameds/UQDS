...
S = range(16) #square number
P = range(4) #Piece number

#Posibility of setting square
Squares = [
...
#Create I[p][i] = set of placement for piece P     
...
for s in S:
    m.addConstr(
        quicksum(X[p,i] for p in P for i in I[p] if s in Squares[p][i])
        ==1)
    
for p in P:
    m.addConstr(quicksum(X[p,i] for i in I[p])==1)
