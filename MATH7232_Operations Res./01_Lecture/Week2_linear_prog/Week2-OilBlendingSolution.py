...
O = range(len(Oils)) #set oil range

# Data
Cost = [110,120,130,110,115]
Hard = [8.8,6.1,2.0,4.2,5.0]
IsVeg = [True,True,False,False,False] #set category
MaxVeg = 200 #constraint
MaxNonVeg = 250 #constraint
Sell = 150 #constraint
MinH = 3 #constraint
MaxH = 6 #constraint
...
X = {}
for i in O:
    X[i] = m.addVar()  #import variable X[n]

# Objective
m.setObjective(quicksum((Sell-Cost[i])*X[i] for i in O), GRB.MAXIMIZE)

m.addConstr(
        quicksum(X[i] for i in O if IsVeg[i]) <= MaxVeg)
